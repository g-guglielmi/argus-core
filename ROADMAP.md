...
- [ ] **Self-hosted / agentless**: AdGuard Home + Home Assistant + UPS via PeaNUT (all HTTP-API — built, on :testing, awaiting lab confirm), DNS name-resolution check (external-check script baked into the argus-probe image; community `dns-resolver.py` + dnspython — next, needs a probe release), Linux SSH (no-SNMP fallback) - _(BE)_ M
