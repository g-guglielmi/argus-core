...
- [ ] **Resolve "latest" from the registry for accurate drift** - Argus core polls GHCR anonymously (`ghcr.io/token` → `/v2/<owner>/argus-probe/tags/list`), picks the newest `X.Y.Z-rN` tag, caches it (every few hours), and compares to each probe's reported version. Lets the fleet view flag "rN available" even when the target is `latest` (today `latest` only shows "tracking"). Package is public, so no auth plumbing. - _(BE+FE)_ S-M
