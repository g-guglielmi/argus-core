...
- [ ] **Delete / deregister a proxy from Argus** - remove a decommissioned probe from the Probes page: `proxy.delete` via the Zabbix API plus cleanup of the Argus-side records (enroll tokens, check-in/version state, per-proxy SNMP defaults). Today a proxy can only be deleted in the Zabbix UI. - _(FE+BE)_ S
