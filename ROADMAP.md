...
- [x] **Self-installing CORE appliance VM** (`deploy/core-vm/`) - the probe golden-image pattern applied to the core: one Packer Debian 13 image with the FULL stack baked (Zabbix 7.0 server + frontend + agent2, PostgreSQL + TimescaleDB pinned 2.28, Docker with `argus`/`argus-updater` pre-pulled, §14c patching in core flavor) and a **one-form first-boot setup page** (`http://<vm>/`: hostname · keymap · timezone · admin email + password) that configures everything behind a live progress page: Debian sudo user, DB init + schema + Timescale, the enrollment **PKI** (works out of the box), `zabbix_server.conf` + frontend conf (**no browser wizard**), stock-`Admin` rotation + **`argus-svc`** super-admin API token (machine-managed, never shown), housekeeping retention, Argus env + first-admin seed + settings seed via API, `:80 → :8081` redirect. One admin password fans out to Debian/Zabbix/Argus (each overridable under Advanced); DB password/token/secret-key generated + never displayed. Manual path stays first-class (`setup-core.sh SETUP_MODE=image` shares the installer; README Option A/B). CI `core-vm.yml`: validate on push, build + Release on `core-vm/v*` tag. See DESIGN **§14d**. **Cut core-vm/v0.1.0 (2026-09-11).** Lab-validated on XCP-NG end to end: one-form setup, sign-in, live Zabbix connection, and a probe enrolled + online over mutual TLS. _(ops+image)_ L
- [x] **Core Zabbix minor updates** (§14c extension, shipped 2026-09-22) - the host reporter adds the
  installed + candidate `zabbix-server-pgsql` version to `os-status.json`; **Settings → OS updates**
  shows core-vs-fleet Zabbix versions and a second operator window (notify-only default) that a new
  host timer (`argus-zbx-update`) honours locally: same-major `zabbix-*` updates only, then a
  `zabbix-server` restart. Majors stay manual (DB migration + Timescale). Existing cores enable it by
...
- [x] Per-site **UniFi controller sweep** (slice 2, shipped) - saved controllers (URL + encrypted
  API key, managed in the Discovery tab), swept from the core or a `sweeps`-capable probe
  (`argus_unifi_sweep.py` / `internal/unifi`); adopted devices arrive with the exact class and the
  four `{$UNIFI.*}` macros injected server-side (the key never reaches the browser). Devices only -
  clients stay the subnet scan's job. See DESIGN §8. - _(BE+FE+probe)_
...
- [x] Default thresholds applied on discovery (adoption reuses `POST /api/hosts`; classes carry the §6 defaults) - _(BE)_ S
- [x] **"Discovered - review"** screen (multi-select confirm / per-row class+macros / ignore with carry-over / already-monitored flagging) - _(FE)_ M
...
- [x] Zabbix client: add `configuration.import`, `template.get`, `host.create`, template/group/macro attach, `usermacro.*` - v0.4.38
- [x] Startup template-reconcile (version the set, import if changed) + `argus/internal/provision/templates/` home - v0.4.38
- [x] Class **registry** (generalize `classifyItem` → class catalog) + `device_class` overlay table keyed by `host_id` - v0.4.38
- [x] Minimal **manual attach** path (`POST /api/hosts` → create host, attach class templates/macros/interface) - the "+ Add device" band in Sites & hosts - v0.4.38
...
- [ ] Finish **SNMP family**: Aruba CX, InstantOn 1960, QNAP, ~~Ugreen UGOS~~ (v0.4.50 - UGOS has no SNMP, so it became the catalog's first **Zabbix-agent** class: passive agent2-in-Docker on the NAS, CPU/mem/fs/NIC + CPU temp + no-wake per-disk SMART temps with a 20 C parked sentinel; lab-confirmed on a DXP4800), ~~unRAID~~ (v0.4.40 - Linux SNMP + extend add-on: per-disk/pool temps, per-share free, per-core CPU; lab-confirmed on two servers; v0.4.49 added CPU temperature via lm-sensors extend + a 20 C parked-drive sentinel), Sophos XGS, NetScaler, Libraesva, ~~Windows (SNMP + LANMGR services)~~ (v0.4.41 - HOST-RESOURCES + per-core CPU + services; lab-confirmed on Win11: 32-bit if-counters, physical-NIC filter, ifAlias naming), Hyper-V (host) - _(BE)_ **L**
...
- [x] **Threshold-banded graphs** - sensor charts colour only the part of the line past a threshold (amber past warning, red past high) instead of tinting the whole chart by the current state, with dashed reference lines and value tags on their own axis. The thresholds come from the sensor's own triggers (host overrides included), so the colouring is retroactive over all history. Group charts band their main channel, and the alert-notification graph uses the same bands and tags. - v0.5.2
