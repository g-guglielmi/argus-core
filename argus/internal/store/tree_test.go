...
func TestTreeOrder(t *testing.T) {
	...
	if err := st.SetTreeOrder(ctx, "site2", "sibling", []string{"h:12", "g:site2/Network"}); err != nil {
		t.Fatalf("sibling set: %v", err)
	}
	sets, _ = st.TreeOrder(ctx)
	var sib []string
	for _, s := range sets {
		if s.Scope == "site2" && s.Kind == "sibling" {
			sib = s.Items
		}
	}
	if len(sib) != 2 || sib[0] != "h:12" || sib[1] != "g:site2/Network" {
