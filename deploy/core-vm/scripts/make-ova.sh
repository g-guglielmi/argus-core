...
# the VMDK, we hand-write the OVF. Same recipe as the probe image (argus-probe repo).
#
...
NAME="${3:-argus-core-vm}"

VMDK="$OUTDIR/$NAME.vmdk"
OVF="$OUTDIR/$NAME.ovf"
OVA="$OUTDIR/$NAME.ova"

# The core runs the whole stack (Zabbix server + PostgreSQL/TimescaleDB + the Argus containers):
# these are the IMPORT DEFAULTS a hypervisor creates the VM with - raise them for bigger fleets.
CPUS=2
MEM_MB=4096
...
<Envelope ovf:version="1.0" xml:lang="en-US"
  ...
  <VirtualSystem ovf:id="$NAME">
    <Info>Argus core appliance golden image</Info>
    <Name>$NAME</Name>
